...
func main() {
	...
	if err := demoBasicUsage(sgq); err != nil {
		log.Printf("basic demo failed: %v", err)
	}

	// example 2: batch processing
	if err := demoBatchProcessing(sgq); err != nil {
		log.Printf("batch demo failed: %v", err)
	}

	// example 3: error handling and retries
	if err := demoErrorHandling(sgq); err != nil {
